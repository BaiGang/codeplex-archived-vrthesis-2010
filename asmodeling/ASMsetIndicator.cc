...
{
  bool ASModeling::set_density_indicator(int level, int * ind_volume)
  {
    int length = (1<<level);
  ...
  bool ASModeling::init_density_volume(int level, std::list<float> & density_vectorized)
  {
    int length = (1<<level);
      ...
      for (int j = 0; j < length; ++j)
      {
        for (int i = 0; i < length; ++i)
        {
          ...
          for (int i_camera = 0; i_camera < num_cameras_; ++i_camera)
          {
            ...
            for (int vv = static_cast<int>(y_min); vv < static_cast<int>(y_max); ++vv)
            {
              for (int uu = static_cast<int>(x_min); uu < static_cast<int>(x_max); ++uu)
              {
                // IMAGE TYPE NOT SPECIFIED YET
                //   USE BMP HERE
                float pix = ground_truth_images_(i_camera, uu, vv);
