...
void PFMImage::SetPixelData(const float* pixel, int width, int height)
{
	...
	{
		MessageBox(NULL, _T("Error: Unknown PFM image type!"), _T("Error"),MB_OK);
	}
...
bool PFMImage::WriteToGrayscalePFM(const char *strFileName)
{
	if(m_pPixel == NULL)
	{
		MessageBox(NULL, _T("Error: There is no image data!"), _T("Error"),MB_OK);
		return false;
	}
...
bool PFMImage::ReadImage(const char *strFileName)
{
	...
	{
		ifs.close();
		MessageBox(NULL, _T("Error: Can not open this PFM file!!"), _T("Error"),MB_OK);
		return false;
	}
	ifs.close();
	return true;
